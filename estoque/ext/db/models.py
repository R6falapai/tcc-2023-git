...
class Items(db.Model):
    ...
    brands_id = db.Column("brands_id", db.Integer, db.ForeignKey("brands.id"))
    available = db.Column("available", db.Boolean)
    stock = db.Column("stock", db.Integer)

    brands = db.relationship("Brands", foreign_keys=brands_id)
...
class Checkout(db.Model):
    ...
    total = db.Column("total", db.BigInteger)
    created_at = db.Column("created_at", db.DateTime)
    completed = db.Column("completed", db.Boolean)
    order_id = db.Column("order_id", db.Integer, db.ForeignKey("order.id"))
